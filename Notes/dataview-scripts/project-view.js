...
(async () => {
    ...
    if (!window[listenerKey]) {
        ...
        // Этот обработчик срабатывает, когда пользователь переключается на вкладку с нашим проектом, гарантируя, что при открытии проекта данные будут на 100% актуальны.
        const leafChangeHandler = (leaf) => {
            if (leaf.view.file && leaf.view.file.path === dv.current().file.path) {
                // Вызываем рендер напрямую, без задержки, т.к. пользователь ожидает увидеть свежие данные.
                render();
            }
        };
